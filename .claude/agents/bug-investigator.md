...
6. Verify what you can: frontend `npx tsc --noEmit -p tsconfig.app.json`, `npm run lint` (must stay 0 problems, blocks CI) and `npm run test` (vitest); backend `python -m pytest -q` (needs `requirements-dev.txt`, so run it in a throwaway `python:3.11-slim` container — no host venv here, PEP 668/no sudo). `GET /health` / `GET /api/health` return `{status, service, environment, revision, ref}` (`ref` = the branch the image was built from); a symptom that only shows up as the wrong `environment`, a stale `revision` or an unexpected `ref` is a config/deploy question (see `env/` and the deploy agent), not a code bug to fix here. Say plainly what is not verified.
7. Hand over: the diff summary and how to verify. Fixes here don't deploy on their own — getting the fix onto `dev.yuriodev.co.uk` needs a commit + a branch push (out of scope for this agent; the repository owner's branch builds go to dev, the last build to finish wins, and only the images change: `env/dev.env` and the compose file stay master's) and, once verified there, a merge to `master` (which moves stage) and a normal release through `/deploy-check` then `/deploy` (release tags, not a per-service on-box build). If two or more approaches failed on the way, draft an `ERRORS.md` entry.
