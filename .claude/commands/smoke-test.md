...
- To confirm a specific version is what's actually running anywhere: `curl -s <origin-or-edge-url>/api/health | python3 -m json.tool` and read `environment` + `revision` (git SHA) + `ref` (the branch it was built from) directly, rather than trusting container uptime alone.
