...
- Environments on this host, one proxy: `yuriodev.co.uk` (root `docker-compose.yml`), `dev.yuriodev.co.uk` (`deploy/dev/compose.yml`, `:dev` = every green master commit) and `stage.yuriodev.co.uk` (`deploy/stage/compose.yml`, `:stage` = a `vX.Y.Z-rc.N` tag via `promote-stage.yml`, or every green master commit while repo variable `STAGE_AUTO_PROMOTE=true`). Dev and stage sit behind basic auth (`nginx-proxy/auth/envs.htpasswd`, gitignored; the plaintext lives in `~/.config/yuriodev/basic-auth.txt`), with noindex and no-store. Their `backend.env` files are gitignored. Service names are environment-unique (`frontend-dev`...); never reuse `frontend`/`backend` in another compose project on `yuriodev-network`.
- Deploy agent: `deploy/agent/yuriodev-deploy.sh` runs from the user crontab every minute with `ENABLED_ENVS`, pulls each environment's images and recreates only services whose image changed, then checks them through the proxy. Log `~/.local/state/yuriodev-deploy.log`, heartbeat `~/.local/state/yuriodev-deploy.heartbeat`. Kill switch: `touch ~/.yuriodev-deploy-paused` (running containers keep running).
