...
- Backend: `.github/workflows/ci.yml`'s `backend` job runs `ruff check`, `ruff format --check`, `mypy` (strict, `src/`) and `python -m pytest -q --cov` with a 90% branch-coverage gate (config in `backend/pyproject.toml`; tools in `requirements-dev.txt`) on Python 3.11, then a Trivy scan of `backend/requirements.txt`'s pins (blocks on fixable HIGH/CRITICAL), covering health GET/HEAD, the error model, docs flag, logging, request ids, CORS, settings parsing, the social feed (`/api/posts` contract and ETag/304, the private-field leak sentinel, the Typefully provider through `httpx.MockTransport`, normalisation, curation, stale/drift guards, snapshot) and every tracked `env/<env>.env` (parses, `ENVIRONMENT` matches the file name, no secret-shaped key with a value). No host venv is possible on this box (PEP 668, no sudo); verify locally in a throwaway `python:3.11-slim` container (see `/deploy-check`).
- `guards` CI job: `scripts/check-compose.py` (image tags per env, container names, `restart: unless-stopped`, `target: runtime`, env-unique service names, the deploy agent's parser view, a logging block on every service, resource limits on every app container, a proxy healthcheck and never a proxy memory limit, no `ports:` on anything but the prod proxy, the proxy image pinned by digest and every proxy volume `:ro`) plus `docker compose ... config --quiet` on every compose file.
- `proxy-image` CI job: Trivy on the digest-pinned proxy image from `docker-compose.yml`; **blocks** on fixable HIGH/CRITICAL CVEs (fix: merge Dependabot's digest bump, then the manual proxy rollout).
