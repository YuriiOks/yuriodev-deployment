...
5. Secrets: never read or print any `env/*.secrets.env`, `backend/.env`, or `deploy/{dev,stage}/backend.env`; use `env/secrets.env.example` (or the legacy `backend/.env.example`) for variable names. Never `docker compose config` without `--quiet`, bare `docker inspect`, or `docker exec ... env`.*
...
- Legacy, about to be deleted (superseded by `env/`, one release cycle out): `backend/.env`, `backend/.env.example`, `deploy/dev/backend.env`, `deploy/stage/backend.env`. Don't extend them.
