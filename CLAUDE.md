...
| `yuriodev-dev` — `deploy/dev/compose.yml` | `frontend-dev`/`yuriodev-dev-frontend`, `backend-dev`/`yuriodev-dev-backend` | `:dev` (every green `master` commit) | basic auth + noindex/no-store; backend env_file `env/dev.env` + optional `env/dev.secrets.env`; limits frontend 96M/0.2 CPU, backend 192M/0.4 CPU |
| `yuriodev-stage` — `deploy/stage/compose.yml` | `frontend-stage`/`yuriodev-stage-frontend`, `backend-stage`/`yuriodev-stage-backend` | `:stage` (a `vX.Y.Z-rc.N` tag, or every green `master` commit while repo variable `STAGE_AUTO_PROMOTE=true`) | basic auth + noindex/no-store; backend env_file `env/stage.env` + optional `env/stage.secrets.env`; limits frontend 96M/0.2 CPU, backend 192M/0.4 CPU |
...
**Images**: each Dockerfile has named stages. Frontend: `deps` -> `dev` (Vite dev server, :5173) / `builder` -> `runtime` (LAST stage; `nginx:stable-alpine`, `apk upgrade`, HEALTHCHECK). Backend: `base` -> `builder` (wheels) -> `dev` (`--reload`, `requirements-dev.txt`) -> `runtime` (LAST stage; non-root uid 10001 `app`, no pip build tooling, HEALTHCHECK via `python -c "urllib.request..."`, `ARG GIT_SHA` -> `ENV REVISION`). Base images are pinned by digest; Dependabot refreshes the pins weekly. Every deployed build (CI, `images.yml`, and break-glass on-box builds) uses `target: runtime`. `GET /health` (internal) and the publicly-routed `GET /api/health` both return `status`, `service`, `environment`, `revision` (git SHA) — use them to prove what is actually running where.
...
curl -s https://yuriodev.co.uk/api/health | python3 -m json.tool          # what's live: status/environment/revision (git SHA)
