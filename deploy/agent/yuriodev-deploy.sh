...
# Quarantine: ~/.local/state/yuriodev-deploy.quarantine/<env>.<service> (the failed digest)
# Optional, each a one-line URL in a file of mode 600 (never in the repo):
#   ~/.config/yuriodev/deploy-alert-url  gets a POST with a short message on a failed deploy
#   ~/.config/yuriodev/deploy-ping-url   gets a GET after every run that logged no failure
# Exit status: 0, or 1 when any enabled environment logged ERROR/FAILED/REFUSED.
set -eEuo pipefail
...
config_url() {  # name -> the URL in ~/.config/yuriodev/<name>, only from a private file
  local f="$CONFIG/$1" mode owner url
  [ -f "$f" ] || return 1
  mode=$(stat -c %a "$f") && owner=$(stat -c %u "$f") || return 1
  if [ "$owner" != "$(id -u)" ] || { [ "$mode" != 600 ] && [ "$mode" != 400 ]; }; then
    log "WARN ignoring $f: needs mode 600 and this user as owner (has $mode, uid $owner)"
    return 1
  fi
  url=$(head -n 1 "$f" | tr -d '[:space:]')
  case "$url" in
    https://*|http://*) printf '%s\n' "$url" ;;
    *) log "WARN ignoring $f: the first line is not an http(s) URL"; return 1 ;;
  esac
}
...
deploy() {  # env file service image container port; sets rc=1 on any failure
  ...
  local repo="${image%:*}" qfile="$QUARANTINE/$1.$3" want have digest key quarantined=""
  ...
  if [ -f "$qfile" ]; then quarantined=$(head -n 1 "$qfile"); fi
  if [ -n "$quarantined" ] && [ "$quarantined" != "$key" ]; then
    rm -f "$qfile" "$qfile.noted"
  ...
  if [ -n "$quarantined" ]; then
    # Point the local tag back at what is running, so a manual `up` cannot start the failed image.
    docker tag "$want" "$held" >/dev/null 2>&1 || true
    if [ -n "$have" ]; then docker tag "$have" "$image" >/dev/null 2>&1 || true; fi
    if [ ! -e "$qfile.noted" ]; then
  ...
  if docker compose -f "$file" up -d --no-deps "$svc" </dev/null >/dev/null 2>&1 && healthy "$container" "$port"; then
  ...
  docker tag "$want" "$held" >/dev/null 2>&1 || true
  if [ -z "$have" ]; then
    log "$env $svc FAILED health check after deploying $digest; no previous container to roll back to; quarantined $key"
    alert "$env $svc FAILED health check after deploying $key; no previous image to roll back to; quarantined"
    return 0
  fi
  log "$env $svc FAILED health check after deploying $digest; rolling back to $have"
  if docker tag "$have" "$image" >/dev/null 2>&1 &&
     docker compose -f "$file" up -d --no-deps --force-recreate "$svc" </dev/null >/dev/null 2>&1 &&
     healthy "$container" "$port"; then
    log "$env $svc ROLLED BACK to $have; quarantined $key"
    alert "$env $svc FAILED health check after deploying $key; rolled back to the previous image; quarantined"
  else
    log "$env $svc ROLLBACK FAILED to $have (still unhealthy); quarantined $key"
    alert "$env $svc FAILED health check after deploying $key, and the rollback to the previous image is unhealthy too; quarantined; needs a human"
  fi
}
